...
- 대상 서버는 설정에서 서버 ID 목록으로 지정하거나, 토글 하나로 모든 서버를 대상으로 삼습니다.
...
- 같은 문장은 캐시해서 재요청하지 않습니다. "번역 불필요" 라는 판정도 함께 캐시됩니다.
- 자동 번역은 단축키(기본 `Ctrl+Shift+T`)로 껐다 켤 수 있습니다.
- **내가 보내는 메시지**를 원하는 언어로 번역해서 내보낼 수도 있습니다(기본 꺼짐, `Ctrl+Shift+O`).
- 새 버전이 저장소에 올라오면 Discord 안에서 자동으로 갱신됩니다.

## 자동 업데이트

BetterDiscord 자체 업데이터는 **공식 애드온 스토어에 등재된 플러그인만** 갱신하므로, 이 플러그인은 직접 확인합니다.

시작 15초 뒤와 이후 4시간마다 `meta.json` 의 `source` 로부터 만든 주소
`https://raw.githubusercontent.com/<owner>/<repo>/main/dist/Mollu.plugin.js` 를 받아, 배너의 `@version` 을 지금 실행 중인 버전과 숫자 단위로 비교합니다. 더 높을 때만 `plugins` 폴더에 덮어쓰며, BetterDiscord 가 파일 변경을 감지해 플러그인을 다시 불러옵니다.

즉 **새 버전을 내보내는 방법은 `meta.json` 의 `version` 을 올리고, 빌드한 `dist/Mollu.plugin.js` 를 `main` 에 푸시하는 것**입니다. 버전을 올리지 않고 푸시하면 아무 일도 일어나지 않습니다. 같은 이유로 로컬에서 개발 중인 빌드가 원격 빌드로 덮어써지지 않습니다.

받은 파일의 배너 이름이 `Mollu` 가 아니거나 5MB 를 넘으면 쓰지 않고, 주소는 `meta.json` 에서만 만들어집니다. 저장소가 없거나 파일이 아직 없으면 조용히 넘어가고 콘솔에만 남습니다.
...
| 모든 서버에서 번역 | 켜면 아래 목록을 무시하고 참여 중인 모든 서버가 대상이 됩니다. DM 은 어느 쪽이든 제외 |
...
| 자동 번역 단축키 | 설정 창을 열지 않고 위 스위치를 껐다 켭니다. **칸을 누른 뒤 원하는 키를 누르면 그대로 등록**됩니다. 기본값 `Ctrl+Shift+T`, 지우면 사용하지 않습니다 |
| 보내는 메시지도 번역 | 켜면 대상 서버에서 **입력한 글을 번역문으로 바꿔** 보냅니다. 상대는 원문을 볼 수 없습니다. 기본 꺼짐 |
| 보낼 때 번역할 언어 | 위 기능이 쓰는 대상 언어. 이미 그 언어로 쓴 메시지는 그대로 나갑니다 |
| 보내는 메시지 번역 단축키 | 위와 같은 방식으로 등록합니다. 기본값 `Ctrl+Shift+O` |
| 봇 메시지도 번역 / 내 메시지도 번역 | 필터 토글 |
| 번역 중 표시 / 번역 실패 시 표시 | UI 토글 |
| **고급** — 번역하지 않은 사유 기록 | 켜면 번역을 건너뛴 이유를 콘솔(`Ctrl+Shift+I`)에 남깁니다. 아무것도 안 나올 때 원인을 확인하는 용도 |
| **고급** — 자동 업데이트 | `meta.json` 의 저장소에서 `dist/Mollu.plugin.js` 를 받아 `@version` 을 비교합니다. 더 높으면 `plugins` 폴더에 덮어쓰고, BetterDiscord 가 파일 변경을 감지해 다시 불러옵니다 |
| **고급** — 업데이트 | `확인` 으로 지금 즉시 검사합니다 |
| **고급** — 번역 캐시 | `비우기` 로 저장된 번역을 모두 삭제합니다. 확인창이 한 번 뜨며, 비운 뒤에는 모든 메시지가 다시 API 로 나갑니다 |
...
src/
  ...
  hotkey.js                  전역 단축키 (자동 번역 / 보내는 메시지 번역 토글)
  outgoing-patch.js          sendMessage 패치 — 보내는 메시지를 번역해서 내보냄
  ui/
  ...
  translation/
    ...
    language-detector.js       대상 언어로 이미 쓰였는지 판정
...
영어·스페인어·프랑스어·독일어·포르투갈어·베트남어·인도네시아어는 **모두 라틴 문자라 보내기 전에 서로 구분할 수 없습니다.** 이 경우 메시지마다 한 번은 API로 보내고, 모델이 원문 그대로 돌려주면(= 이미 대상 언어면) 표시하지 않고 그 판정을 캐시합니다. 이 판정도 디스크에 남으므로 비용은 문장당 한 번뿐이며, 재시작해도 다시 묻지 않습니다.
...
대상 서버에서 대상 언어가 아닌 것으로 판정된 메시지의 **본문 텍스트**가 설정한 API 엔드포인트(기본값 DeepSeek)로 전송됩니다. 대상 서버 외 메시지, 이미 대상 언어인 메시지, 필터로 제외된 메시지는 전송되지 않습니다.

`보내는 메시지도 번역` 을 켜면 **내가 입력한 글도** 보내기 직전에 같은 엔드포인트로 전송됩니다. 이 경우 상대에게는 번역문만 나가고 원문은 남지 않습니다. 번역에 실패하면 원문 그대로 전송되며 토스트로 알립니다.
...
번역 캐시는 마지막 쓰기로부터 10초 뒤에 저장되며, 플러그인을 끌 때도 저장됩니다.
